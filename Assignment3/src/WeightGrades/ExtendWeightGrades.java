...
public class ExtendWeightGrades extends WeightGrades
{
  ...
  /**
   * @MethodName addOneAssignment
   * @Author Liangxi Liu
   * @Description add earned point, total point, percentage of an assignment / exam
   * @Param double _earnedPoint, double _totalPoint, double _percentage;
   */
  public void addOneAssignment(double _earnedPoint, double _totalPoint, double _percentage) {
    this.extendEarnedPoints.add(_earnedPoint);
    this.extendPointTotal.add(_totalPoint);
    this.extendAssignmentPercentage.add(_percentage);
  }
  ...
  {
    assert this.extendEarnedPoints.size()==this.extendAssignmentPercentage.size();
    assert this.extendEarnedPoints.size()==this.extendPointTotal.size();
    double points = 0;
    double totalP = 0;
  ...
  {
    assert _assignmentPercentage.size()==8;
